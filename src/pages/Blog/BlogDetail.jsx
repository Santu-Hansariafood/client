...
import { FaArrowLeft, FaPrint, FaRegCalendarAlt, FaUser } from "react-icons/fa";
        ...
        <div className="p-8 border-b-4 border-double border-slate-900 text-center">
          <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-[0.3em] text-slate-500 mb-4">
            ...
            <div className="flex items-center gap-4">
               <button onClick={() => navigate(-1)} className="hover:text-slate-900 transition-colors flex items-center gap-1">
                 <FaArrowLeft size={8} /> Back
               </button>
               <button onClick={() => window.print()} className="hover:text-slate-900 transition-colors flex items-center gap-1">
          ...
          <div className="border-y border-slate-900 py-2 flex justify-between items-center text-[11px] font-black uppercase tracking-widest text-slate-800">
            ...
            <span>{new Date(blog.date).toLocaleDateString("en-GB", { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' }).toUpperCase()}</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
            ...
            <div className="md:col-span-4 border-l border-slate-200 pl-8 hidden md:block">
              <div className="sticky top-10 space-y-8">
                <div>
                  <h4 className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-3">Posted By</h4>
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center text-slate-400">
                      <FaUser size={14} />
                    </div>
                    <div>
                      <p className="text-sm font-black text-slate-800 uppercase">{blog.author?.name || "Admin"}</p>
                      <p className="text-[10px] font-bold text-slate-500 uppercase tracking-tighter">{blog.author?.role || "Staff"}</p>
                    </div>
                  </div>
                </div>
