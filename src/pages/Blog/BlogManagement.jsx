...
import { FaPlus, FaTrash, FaImage, FaSave, FaNewspaper, FaRegCalendarAlt } from "react-icons/fa";
    ...
    >
      <div className="max-w-6xl mx-auto space-y-6">
        <div className="flex justify-between items-center">
          <h3 className="text-xl font-black text-slate-800 uppercase tracking-tight">News Bulletin</h3>
          <button 
          ...
          >
            {showAddForm ? "Close Form" : <><FaPlus /> Create News</>}
          </button>
        </div>
        ...
        {showAddForm && (
          <div className="bg-white rounded-3xl border border-slate-200 shadow-xl overflow-hidden animate-in fade-in slide-in-from-top-4 duration-300">
            <div className="p-6 border-b border-slate-100 bg-slate-50/50">
              <h4 className="font-black text-slate-800 uppercase tracking-widest text-[11px]">New Publication</h4>
            </div>
            <form onSubmit={handleSubmit} className="p-8 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Publication Date</label>
                  <input 
                    type="date" 
                    value={formData.date}
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Sub-heading (Summary)</label>
                <textarea 
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-4">
                  <div className={`flex-1 h-32 border-2 border-dashed rounded-2xl flex flex-col items-center justify-center transition-all ${formData.imageUrl ? 'border-emerald-200 bg-emerald-50' : 'border-slate-200 bg-slate-50'}`}>
                    {formData.imageUrl ? (
                      <div className="relative w-full h-full p-2">
                        <img src={formData.imageUrl} className="w-full h-full object-contain rounded-lg" alt="Preview" />
                        <button 
                      ...
                      <>
                        ...
                        <span className="text-[10px] font-bold text-slate-400 uppercase">No image selected</span>
                      </>
                    )}
                  </div>
              ...
              <div className="pt-6 border-t border-slate-100 flex justify-end gap-3">
                ...
                >
                  {submitting ? <Loading size="xs" /> : <><FaSave /> Publish News</>}
                </button>
              </div>
            </form>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {loading ? (
            <div className="col-span-full py-20"><Loading size="lg" /></div>
          ) : blogs.length > 0 ? (
            blogs.map((blog) => (
              <div key={blog._id} className="bg-white rounded-3xl border border-slate-200 overflow-hidden shadow-sm hover:shadow-md transition-all flex flex-col">
                <div className="h-40 bg-slate-100 relative">
                  {blog.imageUrl && <img src={blog.imageUrl} className="w-full h-full object-cover" alt="" />}
                  <button 
                ...
                <div className="p-6 flex-1 flex flex-col">
                  ...
                  <h5 className="text-lg font-bold text-slate-800 line-clamp-2 mb-2">{blog.title}</h5>
                  <p className="text-xs text-slate-500 font-medium line-clamp-3 mb-4">{blog.heading}</p>
                  <div className="mt-auto pt-4 border-t border-slate-50 flex items-center justify-between">
                    <span className="text-[10px] font-black uppercase text-blue-600">Published</span>
                    <button 
