        ...
        {summary.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {summary.map((item, idx) => {
              ...
              const periodLabel =
                summaryType === "month"
                  ? monthNames[item._id.period - 1]
                  : `Week ${item._id.period}`;
              const netBalance = (item.received || 0) - (item.sent || 0);

              return (
                <div
                  key={idx}
                  className="bg-white border border-slate-200 rounded-[2.5rem] p-8 shadow-sm hover:shadow-xl transition-all duration-500 group relative overflow-hidden flex flex-col h-full"
                >
                  ...
                  <div className="relative z-10 flex-1">
                    <div className="flex items-center justify-between mb-8">
                      <div className="flex flex-col">
                        ...
                        <h4 className="text-2xl font-black text-slate-900 italic group-hover:text-white transition-colors">
                          {periodLabel}
                        </h4>
                      </div>
                    ...
                    <div className="space-y-6">
                      <div className="flex flex-col">
                        <div className="flex items-center justify-between text-[10px] font-black uppercase tracking-widest text-emerald-600 mb-2">
                          ...
                          <span className="bg-emerald-50 px-2 py-0.5 rounded-lg border border-emerald-100">
                            {item.receivedCount || 0} Entries
                          </span>
                        </div>
                      ...
                      <div className="flex flex-col">
                        <div className="flex items-center justify-between text-[10px] font-black uppercase tracking-widest text-rose-500 mb-2">
                          ...
                          <span className="bg-rose-50 px-2 py-0.5 rounded-lg border border-rose-100">
                            {item.sentCount || 0} Entries
                          </span>
                        </div>
                        <p className="text-2xl font-black text-slate-900 italic tracking-tighter group-hover:text-white transition-colors">
                  ...
                  <div className="mt-8 pt-6 border-t border-slate-100 group-hover:border-white/10 relative z-10">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest group-hover:text-white/60 mb-1">
                          Net Balance
                        </p>
                        <p
                          className={`text-xl font-black italic ${netBalance >= 0 ? "text-emerald-600" : "text-rose-600"} group-hover:text-white transition-colors`}
                        >
                          {netBalance >= 0 ? "+" : ""}Rs.{" "}
          ...
          <div className="py-32 flex flex-col items-center justify-center text-center">
            ...
            <p className="text-sm text-slate-500 font-medium max-w-xs mx-auto mt-2">
              Analytical data will appear here once you select a ledger and
              record transactions.
            </p>
          </div>
        )}
