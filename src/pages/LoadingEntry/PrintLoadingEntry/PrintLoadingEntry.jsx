...
const PrintLoadingEntry = async (data) => {
  ...
  try {
    ...
    // Force address to be exactly 2 lines
    let cAddrLines = wrapText(shipToAddress, 95, 2);
    // Ensure we have exactly 2 lines (if only 1 line, split it)
    if (cAddrLines.length === 1 && cAddrLines[0].length > 50) {
      const words = cAddrLines[0].split(" ");
      const midPoint = Math.floor(words.length / 2);
      const firstLine = words.slice(0, midPoint).join(" ");
      const secondLine = words.slice(midPoint).join(" ");
      cAddrLines = [firstLine, secondLine];
    } else if (cAddrLines.length === 1) {
      // If single short line, keep as is
      cAddrLines = [cAddrLines[0], ""];
    }
    ...
    consigneeBoxHeight += (cAddrLines.length * 4); // address lines (exactly 2 lines)
    ...
    setNormal();
    // Display address on exactly 2 lines
    cAddrLines.forEach((line, idx) => {
      if (line) { // Only draw if line is not empty
