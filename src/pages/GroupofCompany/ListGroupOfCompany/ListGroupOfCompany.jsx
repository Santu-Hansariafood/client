...
const ListGroupOfCompany = () => {
  ...
  const handleDelete = async (index) => {
    ...
    try {
      await axios.delete(
        `http://localhost:5000/api/groups/${groupToDelete._id}`
      );
      const updatedData = filteredData.filter((_, i) => i !== index);
      setFilteredData(updatedData);
      setGroupsData(
        groupsData.filter((group) => group._id !== groupToDelete._id)
      );
  ...
  const rows = paginatedData.map((group, index) => [
    startIndex + index + 1,
    group.groupName,
    <Actions
      key={index}
