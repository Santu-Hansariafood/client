...
import { WiDaySunny, WiRain, WiCloudy, WiHumidity, WiStrongWind, WiThermometer } from "react-icons/wi";
...
const WeatherWidget = () => {
  ...
  useEffect(() => {
    const fetchWeather = async (lat, lon) => {
      ...
      try {
        ...
        // Reverse geocoding to get a friendlier name
        try {
          const geoRes = await fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${lat}&lon=${lon}&zoom=10`);
          const geoData = await geoRes.json();
          const city = geoData.address.city || geoData.address.town || geoData.address.state || "Local Area";
  ...
  const getWeatherIcon = (code, isDay = 1) => {
    if (code === 0) return <WiDaySunny className={isDay ? "text-amber-400" : "text-indigo-300"} />;
    if (code >= 1 && code <= 3) return <WiCloudy className="text-slate-300" />;
    if (code >= 51 && code <= 67) return <WiRain className="text-blue-400" />;
    if (code >= 71 && code <= 86) return <AiOutlineCloud className="text-slate-200" />;
      ...
      >
        ...
        <div className="flex flex-col items-start leading-none">
          <div className="flex items-center gap-1">
            <span className="text-sm sm:text-base font-black text-white tracking-tighter">{weather.temp}°</span>
            <span className="text-[10px] font-bold text-emerald-300/80">C</span>
          </div>
          <span className="hidden xs:block text-[9px] font-black text-emerald-300/60 uppercase tracking-widest">{locationName}</span>
        </div>
      </button>
        ...
        <div className="space-y-8 p-1">
          ...
          <div className="relative group overflow-hidden rounded-[2.5rem] bg-gradient-to-br from-emerald-600 to-blue-700 p-8 text-white shadow-2xl">
            ...
            <div className="relative z-10">
              <div className="flex justify-between items-start mb-10">
                <div>
                  <div className="flex items-center gap-2 text-emerald-100/80 mb-1">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-[0.2em]">{locationName}</span>
                  </div>
                  <h2 className="text-5xl font-black tracking-tighter flex items-start">
                    {weather.temp}<span className="text-2xl mt-2 ml-1 opacity-60">°C</span>
                  </h2>
                </div>
              ...
              <div className="grid grid-cols-3 gap-3">
                <WeatherStat icon={WiHumidity} label="Humidity" value={`${weather.humidity}%`} />
                <WeatherStat icon={WiStrongWind} label="Wind" value={`${weather.wind} km/h`} />
                <WeatherStat icon={WiRain} label="Precip" value={`${weather.precipitation}mm`} />
              </div>
            </div>
          </div>
          ...
          <div className="px-2">
            ...
            <div className="grid grid-cols-1 gap-3">
              {forecast.map((day, i) => (
                <div key={i} className="flex items-center justify-between p-4 rounded-3xl bg-slate-50/50 border border-slate-100 hover:bg-white hover:shadow-xl hover:shadow-slate-200/50 transition-all duration-500 group">
                  <div className="flex items-center gap-5">
                    <div className="text-3xl group-hover:scale-125 transition-transform duration-500">
                      {getWeatherIcon(day.code)}
                    </div>
                    <div>
                      <p className="text-sm font-black text-slate-800 tracking-tight">{i === 0 ? "Today" : day.date}</p>
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">
                        {day.precip > 0 ? `${day.precip}mm precipitation` : "Clear Conditions"}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-4">
                    <div className="text-right">
                      <p className="text-sm font-black text-slate-900 leading-none">{day.maxTemp}°</p>
                      <p className="text-[10px] font-bold text-slate-400 mt-1 leading-none">{day.minTemp}°</p>
                    </div>
                    <div className="w-1.5 h-8 rounded-full bg-slate-100 overflow-hidden flex flex-col justify-end">
                      <div className="w-full bg-gradient-to-t from-emerald-400 to-blue-400 rounded-full" style={{ height: `${(day.maxTemp / 50) * 100}%` }}></div>
                    </div>
                  </div>
                </div>
              ))}
...
const WeatherStat = ({ icon: Icon, label, value }) => (
  <div className="bg-white/10 backdrop-blur-xl rounded-2xl p-3 border border-white/10 flex flex-col items-center gap-1 transition-all hover:bg-white/20">
    ...
    <span className="text-[8px] font-black uppercase tracking-tighter text-white/60">{label}</span>
