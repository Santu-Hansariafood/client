        ...
        <div className="space-y-2">
          ...
          <div className="pt-2 mt-2 border-t border-slate-50">
            <p className="text-[10px] font-bold text-slate-900 flex items-center justify-between">
              <span>Total:</span>
              <span>Rs. {(payload[0].value + payload[1].value).toLocaleString("en-IN")}</span>
            </p>
          </div>
        </div>
      </div>
...
const PaymentAnalyticsChart = ({ days = 30, chartType = "line" }) => {
  ...
  useEffect(() => {
    const fetchAnalytics = async () => {
      setLoading(true);
      try {
        const response = await api.get("/payments/analytics", { params: { days } });
  ...
  if (!data.length) return (
    <div className="h-[350px] flex flex-col items-center justify-center text-slate-400">
      <p className="font-bold uppercase tracking-widest text-xs">No payment data for this period</p>
    </div>
  );
      ...
      <ResponsiveContainer width="100%" height="100%">
        {chartType === "bar" ? (
          <BarChart data={data} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
            ...
            <Bar dataKey="pending" name="Pending" fill="#fbbf24" radius={[4, 4, 0, 0]} animationDuration={1500} />
            <Bar dataKey="received" name="Received" fill="#10b981" radius={[4, 4, 0, 0]} animationDuration={1500} />
          </BarChart>
        ) : (
