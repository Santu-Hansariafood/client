...
const ConfirmBids = () => {
  ...
  const fetchDetails = async () => {
    try {
      ...
      const bidParticipants = participateRes.data.filter((p) => p.bidId === bidId);
  ...
  const handleStatusChange = async (status) => {
    ...
    const confirmAction = window.confirm(`Are you sure you want to ${status.toLowerCase()} this bid?`);
  ...
  const headers = ["Sl No", "Seller Name", "Company", "Phone", "Email", "Rate", "Quantity", "Status", "Action"];
  ...
  const rows = participants.map((p) => [
    ...
    <span
      className={`px-3 py-1 rounded-full text-white text-sm ${
        p.status === "Confirmed" ? "bg-green-500" : p.status === "Rejected" ? "bg-red-500" : "bg-yellow-500"
      }`}
      key={p.phone}
    >
      {p.status}
  ...
  return (
    <div className="p-4">
        ...
        <>
          {bidDetails && (
            <div className="bg-white shadow-lg rounded-lg p-4 mb-4">
              <h3 className="text-lg font-semibold mb-2 text-center">Bid Details</h3>
              ...
              <p><strong>Quantity:</strong> {bidDetails.quantity} MT</p>
              <p><strong>Rate:</strong> ₹{bidDetails.rate}</p>
              <p><strong>Notes:</strong> {bidDetails.notes}</p>
              <p><strong>Bid Date:</strong> {new Date(bidDetails.bidDate).toLocaleDateString()}</p>
            </div>
          )}
      ...
      {isPopupOpen && (
        <PopupBox isOpen={isPopupOpen} onClose={closePopup} title="Confirm Your Bid">
          <div className="text-center">
            <p><strong>Seller Name:</strong> {selectedBid?.sellerName}</p>
            <p><strong>Company:</strong> {selectedBid?.company}</p>
            <p><strong>Rate:</strong> ₹{selectedBid?.rate}</p>
            <p><strong>Quantity:</strong> {selectedBid?.quantity} MT</p>
