    ...
    <div className="relative bg-white border border-emerald-100 rounded-[1.5rem] md:rounded-[2.5rem] p-4 sm:p-6 md:p-8 shadow-sm overflow-hidden group">
      ...
      <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-4 md:gap-8">
        <div className="flex items-center gap-4 md:gap-8">
          <div className="w-12 h-12 sm:w-16 sm:h-16 md:w-28 md:h-28 bg-white rounded-2xl md:rounded-3xl flex items-center justify-center shadow-lg md:shadow-xl shadow-emerald-100 ring-2 md:ring-4 ring-emerald-50/50 p-2 md:p-3">
            <img
              src={logo}
              alt="Hansaria Logo"
          ...
          <div>
            <div className="flex items-center gap-1.5 md:gap-2 mb-0.5 md:mb-2">
              ...
              <p className="text-[8px] md:text-[10px] font-black text-emerald-600 uppercase tracking-[0.2em] md:tracking-[0.3em]">
                Hansaria Seller Portal
              </p>
            </div>
            <h1 className="text-xl sm:text-2xl md:text-4xl font-black text-slate-800 tracking-tighter">
              {greeting},{" "}
              <span className="text-emerald-600 truncate block sm:inline">Mr. {userName?.split(' ')[0] || "Partner"}</span>
            </h1>
        ...
        <div className="flex flex-wrap items-center gap-3 sm:gap-6 lg:border-l lg:border-emerald-100 lg:pl-10">
          <div className="bg-emerald-50/50 border border-emerald-100 px-4 py-3 md:px-10 md:py-6 rounded-[1rem] md:rounded-[2rem] flex-1 sm:min-w-[240px] shadow-inner text-center">
            <p className="text-[8px] md:text-[10px] font-bold text-emerald-600/60 uppercase tracking-widest mb-1 md:mb-2">
              Net Settlement
            </p>
...
const StatCard = memo(({ title, value, unit, icon, colorClass, subtitle, onClick }) => (
  <div 
    onClick={onClick}
    className={`bg-white p-4 md:p-6 rounded-2xl md:rounded-[2rem] border border-slate-100 shadow-sm flex items-center gap-3 md:gap-6 transition-all duration-500 ${onClick ? 'cursor-pointer hover:shadow-xl hover:border-emerald-200 hover:-translate-y-1 focus:outline-none focus:ring-2 focus:ring-emerald-500' : ''}`}
  >
    <div className={`w-10 h-10 md:w-16 md:h-16 rounded-xl md:rounded-2xl flex items-center justify-center ${colorClass} bg-opacity-10 shadow-inner ring-2 md:ring-4 ring-white shrink-0`}>
...
const CommodityItem = memo(({ item, totalQuantity, onAction, actionLabel, type = "commodity" }) => {
  ...
  return (
    <div className="group relative bg-white hover:bg-emerald-50/30 p-4 md:p-6 rounded-[1rem] md:rounded-[1.5rem] border border-slate-100 transition-all duration-300 shadow-sm md:shadow-none">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 md:gap-5">
        <div className="flex items-center gap-3 md:gap-5 flex-1 min-w-0">
          <div className={`h-10 w-10 md:h-14 md:w-14 rounded-lg md:rounded-2xl flex items-center justify-center text-white shrink-0 shadow-sm group-hover:scale-105 transition-transform ${type === 'company' ? 'bg-indigo-700' : 'bg-emerald-700'}`}>
        ...
        <div className="flex items-center justify-between md:justify-end gap-4 md:gap-12 shrink-0">
          <div className="text-left md:text-right">
            <p className="text-[7px] md:text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-0 md:mb-1">Quantity</p>
            <div className="flex items-baseline justify-start md:justify-end gap-0.5 md:gap-1">
              <span className={`text-sm md:text-lg font-black ${type === 'company' ? 'text-indigo-800' : 'text-emerald-800'}`}>{quantity.toFixed(1)}</span>
              <span className="text-[7px] md:text-[10px] font-bold text-slate-400 uppercase">Tons</span>
            </div>
          </div>
          
          <div className="text-right min-w-[100px] md:min-w-[140px]">
            <p className="text-[7px] md:text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-0 md:mb-1">Brokerage</p>
            <div className="flex items-baseline justify-end gap-0.5 md:gap-1.5">
              ...
              <span className={`text-sm md:text-lg font-black ${type === 'company' ? 'text-indigo-800' : 'text-emerald-800'}`}>
      ...
      <AdminPageShell noContentCard onRefresh={refreshDashboard}>
        <div className="min-h-screen bg-[#f8fafc] p-4 sm:p-6 lg:p-10 space-y-6 md:space-y-10 lg:space-y-14 max-w-[1700px] mx-auto">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 md:gap-8 sm:gap-12 items-start">
            ...
            <div className="xl:col-span-6 bg-white rounded-[1.5rem] md:rounded-[2.5rem] border border-emerald-50 shadow-sm overflow-hidden flex flex-col hover:shadow-lg transition-all duration-500">
          ...
          {/* Footer Bar */}
          <div className="bg-emerald-900 p-6 md:p-10 rounded-[2rem] md:rounded-[3rem] shadow-2xl shadow-emerald-200 flex flex-wrap items-center justify-between gap-6 md:gap-12">
            <div className="flex items-center gap-6 md:gap-12">
              <div className="flex flex-col">
                <p className="text-emerald-400 text-[8px] md:text-[10px] font-black uppercase tracking-[0.3em] mb-1 md:mb-2">Account Status</p>
                <div className="flex items-center gap-2 md:gap-3">
                  <div className="h-2 w-2 md:h-3 md:w-3 rounded-full bg-emerald-500 animate-ping"></div>
                  <span className="text-white font-black uppercase tracking-widest text-[10px] md:text-sm">Verified Partner</span>
                </div>
              </div>
            </div>
            <div className="flex items-center gap-2 md:gap-4 w-full sm:w-auto">
              ...
              <button
                onClick={() => navigate("/Supplier-Bid-List", { state: { mobile } })}
                className="flex-1 sm:flex-none px-4 md:px-8 py-3 md:py-4 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl md:rounded-2xl font-black uppercase tracking-widest text-[8px] md:text-xs transition-all shadow-xl shadow-emerald-900/50 flex items-center justify-center gap-2 md:gap-3"
              >
                <FaGavel />
                Live Bidding
              </button>
