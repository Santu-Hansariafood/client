        ...
        <div className="flex items-center gap-3 sm:gap-4 md:gap-8">
          ...
          <div className="min-w-0 flex-1">
            ...
            <h1 className="text-lg sm:text-2xl md:text-4xl font-black text-slate-800 tracking-tighter leading-tight">
              {greeting}, <span className="text-emerald-600 block sm:inline truncate">Mr. {userName?.split(' ')[0] || "Partner"}</span>
            </h1>
        ...
        <div className="flex flex-wrap items-center gap-3 sm:gap-6 lg:border-l lg:border-emerald-100 lg:pl-10 w-full lg:w-auto">
          <div className="bg-emerald-50/50 border border-emerald-100 px-4 py-3 md:px-10 md:py-6 rounded-[1rem] md:rounded-[2rem] flex-1 sm:min-w-[200px] lg:min-w-[240px] shadow-inner text-center">
            ...
            <div className="flex items-baseline justify-center gap-1 md:gap-1.5">
              <span className="text-[8px] md:text-xs font-black text-emerald-600/40">Rs.</span>
...
const StatCard = memo(({ title, value, unit, icon, colorClass, subtitle, onClick }) => {
  ...
  return (
    ...
    >
      <div className={`w-10 h-10 sm:w-12 sm:h-12 md:w-16 md:h-16 rounded-xl md:rounded-2xl flex items-center justify-center ${colorClass} bg-opacity-10 shadow-inner ring-2 md:ring-4 ring-white shrink-0`}>
        <span className={`text-lg sm:text-xl md:text-2xl ${colorClass.replace('bg-', 'text-')}`}>
          {typeof icon === 'function' ? icon({}) : icon}
        </span>
      </div>
      <div className="min-w-0 flex-1">
        <p className="text-[7px] sm:text-[8px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest mb-0.5 md:mb-1 truncate">{title}</p>
        <div className="flex items-baseline gap-0.5 md:gap-1">
          ...
          {unit && <span className="text-[6px] sm:text-[8px] md:text-xs font-black text-slate-400 uppercase tracking-widest shrink-0">{unit}</span>}
        </div>
        {subtitle && <p className="text-[6px] sm:text-[7px] md:text-[9px] text-emerald-600 font-bold mt-0.5 md:mt-1 uppercase tracking-widest truncate">{subtitle}</p>}
      </div>
    </div>
  );
});
    ...
    >
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 md:gap-5">
        <div className="flex items-center gap-3 md:gap-5 flex-1 min-w-0">
          <div className={`h-10 w-10 sm:h-12 sm:w-12 md:h-14 md:w-14 rounded-lg md:rounded-2xl flex items-center justify-center text-white shrink-0 shadow-sm group-hover:scale-105 transition-transform ${type === 'company' ? 'bg-indigo-700' : 'bg-emerald-700'}`}>
            <span className="text-[10px] md:text-sm font-black tracking-tighter italic">
              {(item?._id || "NA").substring(0, 2).toUpperCase()}
            </span>
          </div>
          <div className="min-w-0 flex-1">
            <h4 className={`text-[10px] md:text-sm font-black uppercase tracking-wider truncate ${type === 'company' ? 'text-indigo-800' : 'text-emerald-800'}`}>
            ...
            <div className="flex items-center gap-1.5 md:gap-3 mt-0.5 md:mt-1.5">
              <p className="text-[6px] md:text-[9px] font-bold text-slate-400 uppercase tracking-widest flex items-center gap-1 md:gap-2 truncate">
                <FaHistory className="text-[5px] md:text-[8px]" aria-hidden="true" /> {item?.trips || 0} Trips
              </p>
              <span className="h-0.5 w-0.5 rounded-full bg-slate-200 shrink-0" aria-hidden="true" />
              <span className={`text-[6px] md:text-[9px] font-black uppercase tracking-widest truncate ${type === 'company' ? 'text-indigo-500' : 'text-emerald-500'}`}>
        ...
        <div className="flex items-center justify-between sm:justify-end gap-3 md:gap-12 shrink-0">
          <div className="text-left sm:text-right">
            <p className="text-[6px] md:text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-0 md:mb-1">Quantity</p>
            <div className="flex items-baseline justify-start sm:justify-end gap-0.5">
              <span className={`text-xs md:text-lg font-black ${type === 'company' ? 'text-indigo-800' : 'text-emerald-800'}`}>{quantity.toFixed(1)}</span>
              <span className="text-[6px] md:text-[10px] font-bold text-slate-400 uppercase">Tons</span>
            </div>
          </div>
          
          <div className="text-right min-w-[70px] sm:min-w-[100px] md:min-w-[140px]">
            <p className="text-[6px] md:text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-0 md:mb-1">Brokerage</p>
            <div className="flex items-baseline justify-end gap-0.5 md:gap-1.5">
              <span className="text-[7px] md:text-[10px] font-black text-slate-400">₹</span>
              <span className={`text-xs md:text-lg font-black ${type === 'company' ? 'text-indigo-800' : 'text-emerald-800'}`}>
          ...
          {onAction && (
            ...
            >
              <FaDownload className="text-[10px] md:text-base" aria-hidden="true" />
            </button>
          )}
        </div>
      </div>

      <div className="mt-3 md:mt-5 h-1 md:h-2 w-full bg-slate-100 rounded-full overflow-hidden p-0.5" role="progressbar" aria-valuenow={percentage} aria-valuemin="0" aria-valuemax="100">
        <div
...
const SellerDashboard = () => {
  ...
  const fetchData = useCallback(async (isMounted = true) => {
    ...
    try {
      ...
      const [sellersRes, bidsRes, participateRes, ordersRes, pendingRes, statsRes] =
        await Promise.all([
          api.get(`/sellers?mobile=${mobile}`).catch(e => ({ data: [], error: e })),
          api.get("/bids?status=active").catch(e => ({ data: { data: [] }, error: e })),
          api.get(`/participatebids?mobile=${mobile}&limit=1`).catch(e => ({ data: { total: 0 }, error: e })),
          api.get(`/self-order?sellerMobile=${mobile}&limit=1&page=1&userRole=Seller`).catch(e => ({ data: { total: 0 }, error: e })),
          api.get(`/self-order/pending/list?mobile=${mobile}&userRole=Seller&limit=1&page=1`).catch(e => ({ data: { total: 0 }, error: e })),
          api.get(`/self-order/seller/stats?mobile=${mobile}`).catch(e => ({ data: {}, error: e })),
        ]);
      ...
      setError("Unable to sync dashboard data. Check your connection and try again.");
    } finally {
  ...
  const navigationItems = useMemo(() => [
    { label: "Today's Bids", icon: <FaGavel />, color: "bg-emerald-600", link: "/Supplier-Bid-List", state: { mobile }, count: sellerBidCount },
    { label: "Add Loading", icon: <FaTruckLoading />, color: "bg-emerald-500", link: "/Loading-Entry/add-loading-entry", state: { mobile }, count: "Create" },
    { label: "Order History", icon: <FaBoxOpen />, color: "bg-indigo-600", link: "/manage-order/list-self-order", state: { mobile }, count: orderCount },
    { label: "Pending Sauda", icon: <FaBook />, color: "bg-amber-600", link: "/Loading-Entry/pending-loading-list", state: { mobile }, count: pendingSaudaCount },
    { label: "Loading List", icon: <FaHistory />, color: "bg-blue-600", link: "/Loading-Entry/list-loading-entry", state: { mobile }, count: "Access" },
    { label: "Alerts", icon: <FaBell />, color: "bg-purple-600", isNotif: true, action: () => togglePopup(true) },
    ...
    <AdminPageShell noContentCard>
      <div className="min-h-screen flex items-center justify-center bg-emerald-50/20 p-4 sm:p-6">
        <div className="bg-white p-8 sm:p-12 rounded-[2rem] sm:rounded-[3rem] shadow-2xl text-center max-w-lg border border-emerald-100 w-full">
          ...
          <h2 className="text-2xl sm:text-3xl font-black text-emerald-800 italic uppercase tracking-tighter mb-4">Sync Error</h2>
          <p className="text-emerald-700/60 font-bold leading-relaxed text-sm sm:text-base mb-8">{error}</p>
          <button 
      ...
      <AdminPageShell noContentCard onRefresh={refreshDashboard}>
        <div className="min-h-screen bg-[#f8fafc] p-3 sm:p-6 lg:p-10 space-y-6 md:space-y-10 lg:space-y-14 max-w-[1700px] mx-auto pb-10">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-3 sm:gap-6">
              {navigationItems.map((item, idx) => (
                <StatCard
                  ...
                  onClick={item.action || (() => navigate(item.link, { state: item.state }))}
                />
              ))}
            </div>
          </section>

          {/* Middle Row: Critical Metrics */}
          ...
          <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 md:gap-8 sm:gap-12 items-start">
            ...
            <section className="xl:col-span-6 bg-white rounded-[1.5rem] md:rounded-[2.5rem] border border-emerald-50 shadow-sm overflow-hidden flex flex-col hover:shadow-lg transition-all duration-500" aria-label="Material Intelligence">
              <div className="p-4 md:p-8 border-b border-emerald-50 bg-emerald-50/20 flex items-center justify-between gap-2">
                <div className="flex items-center gap-2 md:gap-4 min-w-0">
                  <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-2xl bg-emerald-600 flex items-center justify-center text-white shadow-lg shadow-emerald-200 shrink-0">
                    <FaChartBar className="text-base md:text-xl" aria-hidden="true" />
                  </div>
                  <div className="min-w-0">
                    <h3 className="text-sm md:text-xl font-black text-emerald-800 uppercase tracking-tight italic truncate">Material <span className="text-emerald-600">Intelligence</span></h3>
                    <p className="text-[7px] md:text-[10px] font-black text-emerald-700/40 uppercase tracking-[0.2em] md:tracking-[0.3em] truncate">Precision Commodity Breakdown</p>
                  </div>
                </div>
                <span className="text-[7px] md:text-[10px] font-black text-emerald-600 bg-white px-2 md:px-4 py-1 md:py-1.5 rounded-full border border-emerald-100 uppercase tracking-widest shadow-sm shrink-0">Sync Active</span>
              </div>
              <div className="p-3 sm:p-4 md:p-8 space-y-3 md:space-y-5" role="list">
                {commodityBreakdown.length > 0 ? (
                  ...
                  <div className="py-12 md:py-24 text-center">
                    <FaBoxOpen className="text-3xl md:text-5xl text-emerald-100 mx-auto mb-4 md:mb-6" aria-hidden="true" />
            ...
            {/* Company Performance */}
            <section className="xl:col-span-6 bg-white rounded-[1.5rem] md:rounded-[2.5rem] border border-indigo-50 shadow-sm overflow-hidden flex flex-col hover:shadow-lg transition-all duration-500" aria-label="Company Performance">
              <div className="p-4 md:p-8 border-b border-indigo-50 bg-indigo-50/20 flex items-center justify-between gap-2">
                <div className="flex items-center gap-2 md:gap-4 min-w-0">
                  ...
                  <div className="min-w-0">
                    <h3 className="text-sm md:text-xl font-black text-indigo-800 uppercase tracking-tight italic truncate">Company <span className="text-indigo-600">Performance</span></h3>
                    <p className="text-[7px] md:text-[10px] font-black text-indigo-700/40 uppercase tracking-[0.2em] md:tracking-[0.3em] truncate">Entity Wise Revenue Insights</p>
                  </div>
                </div>
                <button 
              ...
              <div className="p-3 sm:p-4 md:p-8 space-y-3 md:space-y-5" role="list">
                {companyBreakdown.length > 0 ? (
                  ...
                  <div className="py-12 md:py-24 text-center">
                    <FaBoxOpen className="text-3xl md:text-5xl text-indigo-100 mx-auto mb-4 md:mb-6" aria-hidden="true" />
          ...
          <footer className="bg-emerald-900 p-6 md:p-10 rounded-[2rem] md:rounded-[3rem] shadow-2xl shadow-emerald-200 flex flex-col md:flex-row items-center justify-between gap-6 md:gap-12">
            <div className="flex items-center gap-6 md:gap-12 w-full md:w-auto justify-center md:justify-start">
              <div className="flex flex-col">
                <p className="text-emerald-400 text-[7px] md:text-[10px] font-black uppercase tracking-[0.3em] mb-1 md:mb-2">Account Status</p>
                <div className="flex items-center gap-2 md:gap-3">
                  <div className="h-2 w-2 md:h-3 md:w-3 rounded-full bg-emerald-500 animate-ping shrink-0" aria-hidden="true"></div>
                  <span className="text-white font-black uppercase tracking-widest text-[9px] md:text-sm">Verified Partner</span>
                </div>
              </div>
            </div>
            <div className="flex items-center gap-3 md:gap-4 w-full sm:w-auto">
              <button
                onClick={() => navigate("/Supplier-Bid-List", { state: { mobile } })}
              ...
              >
                <FaChartBar className="text-emerald-400 text-[10px] md:text-base" aria-hidden="true" />
              ...
              >
                <FaGavel className="text-[10px] md:text-base" aria-hidden="true" />
