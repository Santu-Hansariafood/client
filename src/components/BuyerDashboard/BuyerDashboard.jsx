import { Suspense, lazy, useEffect, useState, useMemo, memo, useCallback } from "react";
      ...
      <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-8">
        <div className="flex items-center gap-8">
          ...
          <div>
            <div className="flex items-center gap-2 mb-2">
              ...
              <p className="text-[10px] font-black text-blue-600 uppercase tracking-[0.3em]">Hansaria Buyer Portal</p>
            </div>
            <h1 className="text-3xl sm:text-4xl font-black text-slate-800 tracking-tighter">
              {greeting}, <span className="text-blue-600">{userName || "Partner"}</span>
            </h1>
            <p className="text-slate-500/60 text-sm font-semibold mt-1">
              Managing procurement for <span className="text-slate-800">{companyName || "Verified Buyer"}</span>
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-6 lg:border-l lg:border-blue-100 lg:pl-10">
          <div className="bg-blue-50/50 border border-blue-100 px-10 py-6 rounded-[2rem] min-w-[240px] shadow-inner text-center">
            <p className="text-[10px] font-bold text-blue-600/60 uppercase tracking-widest mb-2">Total Brokerage</p>
...
const StatCard = memo(({ title, value, unit, icon, colorClass, subtitle, onClick }) => (
  ...
  >
    <div className={`w-16 h-16 rounded-2xl flex items-center justify-center ${colorClass} bg-opacity-10 shadow-inner ring-4 ring-white`}>
    ...
    <div>
      ...
      {subtitle && <p className="text-[9px] text-blue-600 font-bold mt-1 uppercase tracking-widest">{subtitle}</p>}
    </div>
  </div>
));
    ...
    <div className="group relative bg-white hover:bg-blue-50/30 p-6 rounded-[1.5rem] border border-slate-100 transition-all duration-300">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-5">
        <div className="flex items-center gap-5 flex-1 min-w-0">
          <div className={`h-14 w-14 rounded-2xl flex items-center justify-center text-white shrink-0 shadow-sm group-hover:scale-105 transition-transform ${type === 'company' ? 'bg-indigo-700' : 'bg-blue-700'}`}>
            <span className="text-sm font-black tracking-tighter italic">
              {item?._id?.substring(0, 2) || "BR"}
            </span>
          </div>
          <div className="min-w-0 flex-1">
            <h4 className={`text-sm font-black uppercase tracking-wider truncate ${type === 'company' ? 'text-indigo-800' : 'text-blue-800'}`}>
              {item?._id || "Other"}
            </h4>
            <div className="flex items-center gap-3 mt-1.5">
              ...
              <span className={`text-[9px] font-black uppercase tracking-widest ${type === 'company' ? 'text-indigo-500' : 'text-blue-500'}`}>
        ...
        <div className="flex items-center gap-8 md:gap-12 shrink-0">
          <div className="text-right">
            ...
            <div className="flex items-baseline justify-end gap-1">
              <span className={`text-lg font-black ${type === 'company' ? 'text-indigo-800' : 'text-blue-800'}`}>{quantity.toFixed(2)}</span>
              <span className="text-[10px] font-bold text-slate-400 uppercase">Tons</span>
            </div>
          </div>
          
          <div className="text-right min-w-[140px]">
            <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-1">Net Brokerage</p>
            <div className="flex items-baseline justify-end gap-1.5">
              <span className="text-[10px] font-black text-slate-400">Rs.</span>
              <span className={`text-lg font-black ${type === 'company' ? 'text-indigo-800' : 'text-blue-800'}`}>
...
const BuyerDashboard = () => {
  ...
  const handleDownloadInvoice = useCallback(async (companyName) => {
    ...
    const toastId = toast.loading(`Generating brokerage report for ${companyName}...`);
    try {
      ...
      downloadFile(blob, `Brokerage_Report_${companyName.replace(/\s+/g, '_')}.pdf`);
  ...
  const navigationItems = useMemo(() => [
    { label: "Market Bids", icon: <FaGavel />, color: "bg-emerald-600", link: "/manage-bids/bid-list", count: "Live" },
    { label: "New Bid", icon: <FaPlus />, color: "bg-blue-600", link: "/manage-bids/buyer", count: "Post" },
    { label: "Order History", icon: <FaBoxOpen />, color: "bg-amber-600", link: "/manage-order/list-self-order", count: stats.totalSaudas },
    { label: "Transactions", icon: <FaHistory />, color: "bg-indigo-600", link: "/buyer/bid-history", count: "View" },
  ], [stats.totalSaudas]);
      ...
      <AdminPageShell noContentCard>
        <div className="min-h-screen flex items-center justify-center bg-blue-50/20 p-6">
          <div className="bg-white p-12 rounded-[3rem] shadow-2xl text-center max-w-lg border border-blue-100">
            ...
            <h2 className="text-3xl font-black text-blue-800 italic uppercase tracking-tighter mb-4">Sync Error</h2>
            <p className="text-blue-700/60 font-bold leading-relaxed">{error}</p>
            <button 
            ...
            <div className="xl:col-span-6 bg-white rounded-[2.5rem] border border-blue-50 shadow-sm overflow-hidden flex flex-col hover:shadow-lg transition-all duration-500">
              <div className="p-8 border-b border-blue-50 bg-blue-50/20 flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="text-xl font-black text-blue-800 uppercase tracking-tight italic">Material <span className="text-blue-600">Intelligence</span></h3>
                    <p className="text-[10px] font-black text-blue-700/40 uppercase tracking-[0.3em]">Precision Commodity Breakdown</p>
                  </div>
                </div>
                <span className="text-[10px] font-black text-blue-600 bg-white px-4 py-1.5 rounded-full border border-blue-100 uppercase tracking-widest shadow-sm">Sync Active</span>
              </div>
              <div className="p-8 space-y-5">
                {stats.commodityBreakdown.length > 0 ? (
                  stats.commodityBreakdown.map((item, idx) => (
                    <CommodityItem key={item?._id || idx} item={item} totalQuantity={stats.totalUnloadingWeight} />
                  ))
                ) : (
                  <div className="py-24 text-center">
                    <FaBoxOpen className="text-5xl text-blue-100 mx-auto mb-6" />
                    <p className="text-blue-800/40 font-black uppercase tracking-[0.2em] text-xs">No intelligence data available</p>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="xl:col-span-6 bg-white rounded-[2.5rem] border border-indigo-50 shadow-sm overflow-hidden flex flex-col hover:shadow-lg transition-all duration-500">
              <div className="p-8 border-b border-indigo-50 bg-indigo-50/20 flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="text-xl font-black text-indigo-800 uppercase tracking-tight italic">Entity <span className="text-indigo-600">Performance</span></h3>
                    <p className="text-[10px] font-black text-indigo-700/40 uppercase tracking-[0.3em]">Company Wise Procurement Insights</p>
                  </div>
                </div>
                <button 
          ...
          <div className="bg-blue-900 p-10 rounded-[3rem] shadow-2xl shadow-blue-200 flex flex-wrap items-center justify-between gap-12">
            <div className="flex items-center gap-12">
              <div className="flex flex-col">
                <p className="text-blue-400 text-[10px] font-black uppercase tracking-[0.3em] mb-2">Account Status</p>
                <div className="flex items-center gap-3">
                  <div className="h-3 w-3 rounded-full bg-emerald-500 animate-ping" aria-hidden="true"></div>
                  <span className="text-white font-black uppercase tracking-widest text-sm">Verified Procurement Partner</span>
                </div>
              </div>
            </div>
            <div className="flex items-center gap-4">
