    ...
    let [payments, total, totalAmountSum, openingBalanceSum] =
      await Promise.all([
        PaymentReceived.find(query)
          ...
          .select(
            "date voucherNumber sellerBillNo ledgerType ledgerId companyId buyerCompany supplierCompany amount claim tds unadjustedAmount paymentMode paymentType entries mappings remarks createdAt",
          )
          .populate("ledgerId", "name sellerName")
          .populate(
            "mappings.loadingEntryId",
            "saudaNo lorryNumber billNumber loadingDate buyerCompany supplierCompany unloadingWeight loadingWeight paidAmount qualityClaims manualClaim manualClaimAmount secondClaim secondClaimRemarks otherCharges otherChargesRemarks bankCharges bankChargesRemarks tds tdsRemarks generalRemarks",
          )
          .lean(),
