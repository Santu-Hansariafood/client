...
const connect = async () => {
  ...
  try {
    ...
    if (err.message && err.message.includes("querySrv")) {
      ...
      console.warn(
        "Please ensure your DNS is configured correctly or consider using the standard 'mongodb://' connection string format from MongoDB Atlas (for Node.js 2.2.12 or later).\n",
      );
    }
    process.exit(1);
  }
